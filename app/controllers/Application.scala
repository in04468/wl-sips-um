...
class Application(val cache: CacheApi, configuration: Configuration) extends Controller with Security with Logging {
  ...
  def googleAnalyticsId : String = configuration.getString("google.analytics.id")
    .getOrElse(throw new SalesforceException("Google analytics ID could not be loaded from configuration"))
  ...
  def environment() = Action { request =>
    val json: JsValue = JsObject(Seq(
      ...
      "googleAnalyticsId" -> JsString(googleAnalyticsId),
